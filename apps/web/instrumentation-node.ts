 ...
 * whose dependencies (better-sqlite3, drizzle) are Node-only and must never be
...
export function start(): void {
  try {
    ...
    // The app must still boot if the scheduler can't start; the loop logs
    // per-tick failures rather than taking the process down.
